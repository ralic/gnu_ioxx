 ...
 *
 * - Development snapshot: <a href="http://git.cryp.to/todo">ioxx-HEAD.tar.gz</a>\n
 *   \n
 *   The most current development snapshot comes directly from the source code
 *   management repository, which can be <a
 *   href="http://git.cryp.to/%3Fp%3Dioxx">browsed online</a> to see the change
 *   history, etc. Users of the <a href="http://git.or.cz/">git</a> utility can
 *   also download that repository to their local machines:\n \n \verbatim git
 *   clone http://git.cryp.to/ioxx/ \endverbatim \n Note that unlike the stable
 *   releases, development snapshots do not contain the whole zoo of build
 *   scripts necessary to compile the code -- like \c configure. Those scripts
 *   can be generated with the command <code>autoreconf -i</code> if <a
 ...
 * <hr>
 *
 * \section updates How to be notified of new versions
 *
 * If you would like to be notified whenever a new version becomes available,
 * subscribe to the ioxx <a href="http://git.cryp.to/%3Fp%3Dioxx%3Ba%3Drss">RSS
 * feed</a> or <a href="http://git.cryp.to/%3Fp%3Dioxx%3Ba%3Datom">ATOM
 * log</a>.
 *
 * <hr>
 *
